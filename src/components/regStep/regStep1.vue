      ...
      <div class="flex-wrp reg-container">
        <el-form class="form" :model="form" :rules="rules" ref="form" label-width="100px">
          <el-form-item label="所属地区" prop="area">
            <el-cascader v-model="form.area" :options="areaList" clearable :props="searchProps"></el-cascader>
          </el-form-item>
          <el-form-item label="邀请码">
            <el-input v-model="form.invitedCode" placeholder="非必填"></el-input>
          </el-form-item>
          ...
          <el-form-item class="rember-item">
            <el-checkbox v-model="agreeBtn">阅读并同意</el-checkbox><a href="javascript:;">《平台协议》</a>
          </el-form-item>
          <el-form-item>
        ...
        <div class="flex-item reg-notice">
          <h2>注册须知</h2>
          <p>1.请填写自己的手机号，方便下次登录时使用 <br/>2.邀请码为非必填项，填写邀请码是为给您分配责任业务员使用 <br/>2.注册过程中如有疑问，请电话联系：027-84653162</p>
        </div>
      </div>
  ...
  methods:{
    ...
    getCode(){
      if (!this.isCountDown) {
        ...
        API.smsCode({phone: that.form.phone, timestamp: that.timestamp,randomString: that.form.randomString}).then((result)=> {
          if (result.code === 0) {
